...
const uolScript = `<script>const uolList=document.querySelector('#uol-list');fetch('/api/uol').then((r)=>r.json()).then((data)=>{if(!data.items?.length){uolList.innerHTML='<small>Nenhuma manchete disponível agora.</small>';return}uolList.innerHTML=data.items.map((item)=>'<div class="uol-item"><a href="'+encodeURI(item.url)+'" target="_blank" rel="noreferrer">'+item.title+'</a>'+(item.summary?'<small>'+item.summary+'</small>':'')+'<small>Matéria visitada · comentários disponíveis somente no UOL</small></div>').join('')}).catch(()=>{uolList.innerHTML='<small>Manchetes indisponíveis no momento.</small>'});</script>`;
