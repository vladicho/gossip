...
const uolSection = `<section class="uol-panel"><h2>Manchetes do UOL</h2><p>Links públicos para as notícias mais recentes. Classificação automática: política ou não política. Os comentários do UOL são exclusivos para assinantes; por isso, não são copiados nem exibidos aqui.</p><div class="uol-list" id="uol-list"><small>Carregando manchetes...</small></div></section>`;
const uolScript = `<script>const uolList=document.querySelector('#uol-list');const political=/\b(política|político|governo|presidente|congresso|senado|câmara|eleição|eleitoral|partido|lula|bolsonaro|ministro|ministra|deputado|deputada|senador|senadora|vereador|vereadora|prefeito|prefeita|stf|supremo|moraes|pt|pl)\b/i;const classify=(item)=>item.category|| (political.test((item.title||'')+' '+(item.summary||'')+' '+(item.url||''))?'política':'não política');fetch('/api/uol').then((r)=>r.json()).then((data)=>{if(!data.items?.length){uolList.innerHTML='<small>Nenhuma manchete disponível agora.</small>';return}uolList.innerHTML=data.items.map((item)=>{const category=classify(item);return '<div class="uol-item"><span class="uol-tag '+(category==='política'?'politica':'nao-politica')+'">'+category+'</span><a href="'+encodeURI(item.url)+'" target="_blank" rel="noreferrer">'+item.title+'</a>'+(item.summary?'<small>'+item.summary+'</small>':'')+'<small>Matéria visitada · comentários disponíveis somente no UOL</small></div>'}).join('')}).catch(()=>{uolList.innerHTML='<small>Manchetes indisponíveis no momento.</small>'});</script>`;
